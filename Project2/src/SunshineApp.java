...
public class SunshineApp {
    ...
    while (addMoreFiles == 0) {
      ...
      while (f == null && !cancel) {
        ...
        if (f != null) {
          ...
          if(checkFile.exists()) {
            this.files.add(checkFile);
            label.setText("");
            addMoreFiles = openContinuePrompt();
            if (addMoreFiles == 0) {
              f = null;
            }
          } else {
  ...
  /**
   * Opens a prompt asking if they want to continue inputing files.
   *
   * @return int that would be if they want to input another file.
   */
  private int openContinuePrompt() {
    LayoutManager layout = new FlowLayout();
    panel.setLayout(layout);
    int result = JOptionPane.showConfirmDialog(
      frame,
      "Do you want to add another election file?",
      "",
  ...
  private String mutualDisplay(Boolean media){
    ...
    if (!isIR) {
      ...
      results += "The number of valid ballots: "+ myElection.getNumValidBallots() + newline;
    }
   ...
   * Runs the election.
   *
   * @param args This is required in java main functions.
   */
